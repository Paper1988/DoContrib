		...
		>
			<AnimatePresence>
				<motion.div
					...
					className="fixed bottom-6 left-6 z-[999] max-w-[280px] pointer-events-none"
				>
					<div className="pointer-events-auto relative p-5 rounded-[32px] border dark:border-white/10 border-gray-200 dark:bg-black/60 bg-white/80 backdrop-blur-3xl shadow-2xl overflow-hidden group">
						...
						<div className="relative flex gap-3 items-start">
							<div className="mt-1.5 flex-shrink-0 w-2 h-2 rounded-full bg-yellow-500 animate-pulse shadow-[0_0_12px_rgba(234,179,8,0.6)]" />
			...
			<footer className="relative border-t overflow-hidden dark:bg-black dark:border-white/10 bg-[#fdfbfa] border-gray-200 text-gray-900">
				<div className="absolute inset-0 pointer-events-none dark:bg-grid-white/[0.02] bg-grid-gray-900/[0.01]" />

				<div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
					<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-8 sm:gap-10 md:gap-12 mb-8 sm:mb-12">
						<div className="sm:col-span-2">
							<h3 className="text-2xl sm:text-3xl font-bold mb-3 sm:mb-4 dark:text-white text-gray-950">
								DoContrib
							</h3>
							<p className="text-sm leading-relaxed mb-4 sm:mb-6 max-w-sm dark:text-gray-400 text-gray-600 font-medium">
							...
							<div className="flex gap-3">
								{[
									...
									<a
										key={social.label}
										href={social.href}
										className="w-10 h-10 rounded-2xl border flex items-center justify-center transition-all duration-200 dark:bg-white/5 dark:border-white/10 dark:hover:bg-white/10 bg-white border-gray-200 hover:bg-gray-100"
						...
						{/* Footer 連結維持不變 */}
						{[
							{ title: '產品', links: [{ name: '功能', href: '#' }] },
							{ title: '法律', links: [{ name: '隱私權政策', href: '/privacy-policy' }] },
						].map((section) => (
								...
								<ul className="space-y-2">
									{section.links.map((link) => (
										<li key={link.name}>
											<a
												href={link.href}
												className="text-sm dark:text-gray-400 text-gray-600 hover:text-blue-500 transition-colors"
											>
												{link.name}
											</a>
										</li>
					...
					<div className="pt-8 border-t flex flex-col md:flex-row justify-between items-center gap-4 border-gray-200 dark:border-white/10">
						<p className="text-xs font-bold tracking-[0.2em] uppercase dark:text-gray-500 text-gray-400">
